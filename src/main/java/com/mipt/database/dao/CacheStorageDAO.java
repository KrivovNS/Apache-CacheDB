...
public class CacheStorageDAO {
  ...
  public CacheStorage loadDataFromDatabaseByToken(String storageToken) {
    ...
    try (Connection conn = DatabaseConnection.getConnection()) {
      ...
      List<String> tableNames = getStorageTables(storageToken);
  ...
  private List<String> getStorageTables(String storageToken) throws SQLException {
    ...
    try (Connection conn = DatabaseConnection.getConnection();
      ...
      stmt.setString(1, storageToken + "_%");
  ...
  private DataType extractDataTypeFromTableName(String storageToken, String tableName) {
    try {
      ...
      return DataType.fromValue(typePart);
    } catch (Exception e) {
    ...
    try (PreparedStatement stmt = conn.prepareStatement(selectSQL);
      ...
      while (rs.next()) {
        ...
        if (key != null && value != null) {
          cacheStorage.put(dataType, key, value);
          loadedCount++;
        }
      }
